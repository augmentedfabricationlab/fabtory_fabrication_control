...
def move_to_joints_2(robot, configuration, speed=250, zone=rrc.Zone.FINE, send_and_wait=False):
    """
    ...
    if send_and_wait:
        robot.abb_client.send_and_wait(rrc.MoveToJoints(joints, cart, speed, zone)) #send move to joints command
    else:
