...
from compas.geometry import intersection_line_line_xy, distance_point_point, is_point_in_polygon_xy, rotate_points, rotate_points_xy
...
def floorslab_creation(self):
    ...
    def __board_data_setup(advanced):
        def element_data_creator(dims, lay, global_c, layer_c, z, len_dir, w_dir, grid_pos, vert_support, perpendicular=False, loc="centre"):
            ...
            myElement.final_glue_frames = []
            myElement.assembly_path = []
...
def gluepoints(system, snake = False, glue_station_default_frame = None, safety_distance = None, safety_distance_gluepoints = None, dryrun = False,
...
primary_interval = 9.5
primary_falloff = 100.0
primary_dedensification = 2
secondary_interval = 34.0
secondary_interval_development = 1.5
skip_centrals = True
...
gluegun_station_frame = Frame(Point(3.5, 0, 2), Vector(0, 1, 0), Vector(-1, 0, 0))
stack_creator(Slabassembly, stack_frame)
gluepoints(Slabassembly, snake=False, glue_station_default_frame=gluegun_station_frame, safety_distance=0.2, safety_distance_gluepoints=0.04,
           flipped_dropframe_distance = 1.5)
